...
if (localStorage.getItem('todoDataList1')){
  data = JSON.parse(localStorage.getItem('todoDataList1')); //データを取り出してdataに代入
}else{
  data = [];
}

if (localStorage.getItem('doneList1')){
  done = JSON.parse(localStorage.getItem('doneList1')); //データを取り出してdataに代入
}else{
  done = [];
}
...
let input_text = document.querySelector('#input').value;
  ...
  if (input_text != ''){
    ...
    // 配列をローカルストレージに保存
    localStorage.setItem('todoDataList1',JSON.stringify(data));
...
function add_li_tag(task_text,parent_class_name){
  ...
  if (parent_class_name == '.todo-list'){
    // iタグの生成
    let i_tag_check = create_i('fas fa-check-square');
    let i_tag_trash = create_i('fas fa-trash-alt');

    i_tag_check.classList.add('icon');
    i_tag_trash.classList.add('icon2');
    ...
    i_tag_trash.addEventListener('click',function(){
      click_btn_delete(this,'todoDataList1');
    });
    ...
    // 削除ボタンの処理の設定
    i_tag_archive.classList.add("icon3");
...
function click_btn_check(clicked_btn){
  ...
  // 配列をローカルストレージに保存
  localStorage.setItem('doneList1',JSON.stringify(done));
